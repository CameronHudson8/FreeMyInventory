...
function FMI.upgradeData()
    if  FMI.data.showdebug == nil then  FMI.data.showdebug=false end

    if  FMI.data.itemData == nil then  FMI.data.itemData={} end
    if  FMI.data.tooltips == nil then FMI.data.tooltips={session=true, history=true, recommend=true} end

    if  FMI.data.search_results == nil then FMI.data.search_results={} end
    if  FMI.data.last_search == nil then FMI.data.last_search="" end

    if  FMI.data.language==nil then FMI.data.language="Auto" end
        ...
        if #FMI.ResultControls == 0 then
            ...
            for i = 1,10 do
                local control = CreateControlFromVirtual(
                    "FreeMyInventoryResult", FreeMyInventoryResultsBG, "FreeMyInventoryResult", i)
            ...
            toggleButton:SetHandler("OnMouseDown", function() FMI.PopulateGuildList() FMI.GuildStoreSearchToggle() end)
        end
...
function FMI.Reset()
    if  FMI.Terms ~= nil then FreeMyInventoryTerms:SetText("") end
...
function FMI.SetResultLine(index, name, count, price, unitPrice, guildId, pageNumber, icon, quality, gName)
    ...
    resultControl:SetText(string.format(FMI.lang.gui.res_label,icon,gName, FMI.utils.GetQualityColor(quality), name, count))
    resultControl:GetNamedChild("Price"):SetText(string.format(FMI.lang.gui.res_price,price))
    resultControl:GetNamedChild("UnitPrice"):SetText(string.format(FMI.lang.gui.res_unitprice,string.format("%.2f", unitPrice)))
...
function FMI.OnSliderMouseWheel(self, delta)
    if #FMI.SearchMatches <= #FMI.ResultControls then return end
    ...
    for i = 1, numItemsOnPage do
        ...
        then
          ...
          local currentResult = {
			...
			FMI.utils:NameCleanupLower(name).."::"..quality,--[13]	itmIndex
          ...
          FMI.data.itemData[itmIndex].session[gname].seen=FMI.data.itemData[itmIndex].session[gname].seen+stackCount
  
          FMI.data.itemData[itmIndex].session[gname].sum=FMI.data.itemData[itmIndex].session[gname].sum+price
  
          FMI.data.itemData[itmIndex].session[gname].avg=FMI.data.itemData[itmIndex].session[gname].sum/FMI.data.itemData[itmIndex].session[gname].seen
          ...
          if FMI.data.itemData[itmIndex].history[gname].min==-1 or FMI.data.itemData[itmIndex].history[gname].min>itmCost then
              FMI.data.itemData[itmIndex].history[gname].min=itmCost
          end
  
          if FMI.data.itemData[itmIndex].history[gname].max==-1 or FMI.data.itemData[itmIndex].history[gname].max<itmCost then
          ...
          FMI.data.itemData[itmIndex].history[gname].seen=FMI.data.itemData[itmIndex].history[gname].seen+stackCount
  
          FMI.data.itemData[itmIndex].history[gname].sum=FMI.data.itemData[itmIndex].history[gname].sum+price
  
          FMI.data.itemData[itmIndex].history[gname].avg=FMI.data.itemData[itmIndex].history[gname].sum/FMI.data.itemData[itmIndex].history[gname].seen
        else
          FMI.debug("--------------------------------------")
...
function FMI.ComputeSessionStandardDeviations()
	...
	for i1, v in ipairs(FMI.data.search_results) do
		...
		FMI.data.itemData[itmIndex].session[gname].stdev = FMI.data.itemData[itmIndex].session[gname].stdev + (itmCost - avg)^2 * stackCount
	end
...
function FMI.GetItemStatistics(itemID, itemIndex, skipDots, goBack, clickable)
  ...
  if MasterMerchant.salesData[itemID] and MasterMerchant.salesData[itemID][itemIndex] and MasterMerchant.salesData[itemID][itemIndex]['sales'] and #MasterMerchant.salesData[itemID][itemIndex]['sales'] > 0 then
	--d("Passed initial checks...")
    local list = MasterMerchant.salesData[itemID][itemIndex]['sales']
    local lowerBlacklist = MasterMerchant:ActiveSettings().blacklist and MasterMerchant:ActiveSettings().blacklist:lower() or ""
    local timeCheck, daysRange = MasterMerchant:TimeCheck()
  
	--d("Performing timecheck...")
    if timeCheck == -1 then return 0, 0 end
...
function FMI.ComputeBestItemsToSell(total_inv_counts, total_inv_links)
	...
	for i, v in pairs(total_inv_links) do
		...
		if item_data[i]['avgPrice'] == nil then item_data[i]['avgPrice'] = 0 end
		if item_data[i]['stdDev'] == nil then item_data[i]['stdDev'] = 0 end
		...
		if item_data[i]['samples'] > 0 then -- compute probability density function
			...
			while peak_reached == false do
				...
				if item_data[i]['stdDev'] > 0 then -- use normal distribution
					...
					y[index] = 	math.pow(2 * math.pi * math.pow(item_data[i]['stdDev'],2),-0.5) *  math.exp(-math.pow(x - item_data[i]['avgPrice'],2) / (2 * math.pow(item_data[i]['stdDev'],2) ) )
									
				elseif item_data[i]['stdDev'] == 0 then -- use t distribution
					
					local DoF = item_data[i]['samples'] - 1
					if DoF == 0 then DoF = 1 end
					y[index] = 	FMI.GammaFunc((DoF+1)/2) * math.pow((1+math.pow(x - item_data[i]['avgPrice'], 2)/DoF),-(DoF+1)/2) / ( math.pow(DoF * math.pi, 0.5) * FMI.GammaFunc(DoF/2) )
...
function FMI.GuildStoreOpened(event)
	...
	local item_data_keys = FMI.SortByKeys(item_data, function(a, b) return a < b end)
	
	for _, key in ipairs(item_data_keys) do
		d(string.format("%d x %s @ %.2f = %d", total_inv_counts[key], key, item_data[key]['optimalPrice'], item_data[key]['totalValue']))
	end
...
function FMI.PopulateGuildList()
    if FMI.dropDownInit then return end
    ...
    local entry = dropdown:CreateItemEntry(FMI.lang.gui.guild_all, function() end)
    ...
    while #FMI.Guilds < numGuilds do
        ...
        if resultId ~= 0 then
            ...
            local entry = dropdown:CreateItemEntry(string.format(FMI.lang.gui.guild_templ, resultId, color, name), function() end)
            dropdown:AddItem(entry)
...
function FMI.FindMatches()
    if FMI.data.search_results == nil or table.getn(FMI.data.search_results) < 1 then return end
    ...
    for i = 1, numberOfItems do
        ...
        if currentGuild == FMI.lang.gui.guild_all then currentGuild = guildId end
    ...
    FreeMyInventoryMatchCounter:SetText(string.format(FMI.lang.gui.search_summ,matchCount,math.floor(unitcost.max),math.floor(unitcost.min),math.floor(unitcost.avg),math.floor(unitcost.undercut),sellFor))
    -----  End Added by LintyDruid
...
function FMI.SetTooltip(itemData)
    ...
    if next(FMI.data.itemData[itmIndex].session)~=nil  then
        ...
        for id,data in pairs(FMI.data.itemData[itmIndex].session) do
            if FMI.data.tooltips.session then -- show session
                table.insert(tt_text,string.format(FMI.lang.gui.tt_line,shortguild(id),numfmt(data.min),numfmt(data.max),numfmt(data.avg),numfmt(data.min*((100-FMI.data.undercutPerc)/100))))
            end
    ...
    if FMI.data.tooltips.recommend then -- show recommendations
        ...
        if sess_bp>-1 then
            table.insert(tt_text,string.format(FMI.lang.gui.tt_Sell,sess_g,numfmt(sess_bp*((100-FMI.data.undercutPerc)/100)*qty),qty_lbl,FMI.lang.gui.tt_sessshort))
        else
            table.insert(tt_text,string.format(FMI.lang.gui.tt_Sell,hist_g,numfmt(hist_bp*((100-FMI.data.undercutPerc)/100)*qty),qty_lbl,FMI.lang.gui.tt_histshort))
        end

        if sess_bb>-1 then
            table.insert(tt_text,string.format(FMI.lang.gui.tt_Buy,sess_gb,numfmt(sess_bb*qty),qty_lbl,FMI.lang.gui.tt_sessshort))
        else
            table.insert(tt_text,string.format(FMI.lang.gui.tt_Buy,hist_gb,numfmt(hist_bb*qty),qty_lbl,FMI.lang.gui.tt_histshort))
        end
    end
...
EVENT_MANAGER:RegisterForEvent("FreeMyInventory", EVENT_SCREEN_RESIZED, function(...) 
