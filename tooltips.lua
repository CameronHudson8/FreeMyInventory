...
local function register()
    ...
    ZO_PreHookHandler(
        ...
        "OnShow",
        function(control, ...)
            ld_tooltip.hooks.ToolTipShowHandler(control)
        end
    )
    ZO_PreHookHandler(
        PopupTooltip,
        "OnUpdate",
        function(control, ...)
            ld_tooltip.hooks.TooltipUpdateHandler(control)
        end
    )
    ZO_PreHookHandler(
        PopupTooltip,
        "OnHide",
        function(control, ...)
            ld_tooltip.hooks.ToolTipHideHandler(control)
        end
    )
    ZO_PreHookHandler(
        PopupTooltip,
        "OnAddGameData",
        function(control, ...)
            ld_tooltip.hooks.ToolTipAddGameDataHandler(control)
        end
    )
    --ItemTooltips
    ZO_PreHookHandler(
...
local function additemtooltip(name, sectionLabel, callback)
    ...
    if (ld_tooltips.items[name] == nil) then
        ld_tooltips.items[name] = {}
    end
...
local function DissectItemLink(itemLink)
    if (itemLink) then
        ...
        if (itemQuality == nil) then
            itemQuality = inferQualityFromLinkColor(itemColor)
        end
        if itemID == nil then
            itemID = -1
        end
        ...
        if itemText:sub(1, 1) == "[" then
            itemText = itemText:sub(2)
        end
        if itemText:sub(itemText:len()) == "]" then
            itemText = itemText:sub(1, itemText:len() - 1)
        end

        local itemIcon, itemSellPrice, itemMeetsUsageRequirement, itemEquipType = GetItemLinkInfo(itemLink)
...
local function ToolTipShowHandler(control)
    ...
    if (control == ItemTooltip) then
        ...
        if (mouseOverControl ~= nil) then
            ...
            if (mocParent) then
                if
                    (mouseOverControl.dataEntry and mouseOverControl.dataEntry.data.bagId and
                        mouseOverControl.dataEntry.data.slotIndex)
                 then --is inventroy
                    ...
                    ld_tooltip.currLink = {
                        source = ld_tooltips.sources.inventory,
                ...
                elseif (mouseOverControl:GetParent():GetName() == "ZO_StoreWindowListContents") then --is store item
                    ...
                    ld_tooltip.currLink = {
                        source = ld_tooltips.sources.store,
                        name = mouseOverControl.dataEntry.data.name,
                        quality = mouseOverControl.dataEntry.data.quality,
                        icon = mouseOverControl.dataEntry.data.icon,
                        link = curMouseOverLink,
                ...
                elseif (mouseOverControl:GetParent():GetName() == "ZO_BuyBackListContents") then --is buyback item
                    ...
                    ld_tooltip.currLink = {
                        source = ld_tooltips.sources.buyback,
                        name = mouseOverControl.dataEntry.data.name,
                        quality = mouseOverControl.dataEntry.data.quality,
                        icon = mouseOverControl.dataEntry.data.icon,
                        link = curMouseOverLink,
                ...
                elseif (mouseOverControl:GetParent():GetName() == "ZO_InteractWindowRewardArea") then --is reward item
                    ...
                    ld_tooltip.isNew = true
                elseif
                    (mouseOverControl.dataEntry and mouseOverControl.dataEntry.data.name and
                        mouseOverControl.dataEntry.data.quality and
                        mouseOverControl.dataEntry.data.icon)
                 then -- is partial store data
                    ld_tooltip.currLink = {
                        source = ld_tooltips.sources.auctionItem,
                        name = mouseOverControl.dataEntry.data.name,
                        quality = mouseOverControl.dataEntry.data.quality,
                        icon = mouseOverControl.dataEntry.data.icon,
                        link = nil,
...
--- Demo Test Function
local function ld_testfunction(
    itemID,
    itemText,
    itemQuality,
    itemIsBound,
